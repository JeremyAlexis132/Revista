...
def extraer_contenido(html_path: str) -> ContenidoArticulo:
    ...
    for elem in elementos:
        ...
        if not es_fecha and has_date_kw and (has_digits or len(texto_limpio) < 40) and not is_email_or_inst: es_fecha = True
        ...
        if "orcid" in clases_limpias:
            if autor_actual:
                ...
                if enlace and enlace.get("href"): autor_actual.orcid_url = enlace["href"].strip()
                else:
        ...
        if ("resumen" in clases and "resumen_ingles" not in clases) or texto_lower.startswith("resumen:"):
            contenido.resumen = str_elem; fase = "cuerpo"; continue
        if "palabras-clave" in clases or texto_lower.startswith("palabras clave:"):
            contenido.palabras_clave = str_elem; continue
        if "resumen_ingles" in clases or texto_lower.startswith("abstract:"):
            contenido.abstract = str_elem; continue
        if "keywords" in clases or texto_lower.startswith("keywords:"):
            contenido.keywords = str_elem; continue
        if "referencias" in clases or "bib" in clases_limpias:
            contenido.referencias.append(str_elem); fase = "referencias"; continue
        if "como_citar" in clases or "iijunam" in clases_limpias or "apa" in clases_limpias or texto_lower == "cómo citar":
            contenido.como_citar.append(str_elem); fase = "como_citar"; continue
        if fase == "como_citar":
            if elem.name == "hr": fase = "cuerpo" 
            else: contenido.como_citar.append(str_elem)
            continue

        if texto_limpio or elem.name in ["table", "img", "hr"]:
            if elem.name == "table": str_elem = f'<div class="table-responsive">\n{elem}\n</div>'
...
def generar_html_referencia(contenido: ContenidoArticulo, css_inline: str, nombre_revista: str) -> str:
    ...
    # SVG Configurado correctamente para alinearse al texto
    ...
    for autor in contenido.autores_obj:
        # Limpieza robusta del nombre del autor (evita herencias de InDesign)
        ...
        # La clase "autor_item" inyectará el margen superior (espaciado o "enter") SÓLO por encima del nombre
        autores_html_list.append(f'<div class="autor_bloque">')
        autores_html_list.append(f'\t<p class="autor_item">{nombre_final}{orcid_str}</p>')
        ...
        autores_html_list.append(f'</div>')
