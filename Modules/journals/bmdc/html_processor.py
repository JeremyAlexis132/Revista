...
Garantiza la limpieza estricta de saltos de línea para mantener el ORCID 
en la misma línea del autor y asegurar una correcta estructuración.
"""
...
def extraer_contenido(html_path: str) -> ContenidoArticulo:
    ...
    for p in soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'div']):
        for text_node in p.find_all(string=True):
            ...
            if text_node != nuevo_texto:
                text_node.replace_with(nuevo_texto)

    for p in soup.find_all('p'):
        if not p.get_text(strip=True) and not p.find(['img', 'table', 'span']):
            p.decompose()
    ...
    for elem in elementos:
        ...
        if ("titulo_espanol" in clases or "titulo" in clases_limpias) and "ingles" not in clases:
            if not contenido.titulo_es:
                if elem.name != "h1":
                    elem.name = "h1"
                elem['class'] = elem.get('class', []) + ['titulo_espanol']
        ...
        if "titulo_ingles" in clases or "subtitulo" in clases_limpias:
            if not contenido.titulo_en:
                if elem.name != "h2":
                    elem.name = "h2"
                elem['class'] = elem.get('class', []) + ['titulo_ingles']
                contenido.titulo_en = str(elem)
            continue

        if "aut-dos-nombres" in clases or "aut" in clases_limpias or "autor" in clases_limpias:
            if autor_actual:
        ...
        if "referencias" in clases or "bib" in clases_limpias:
            contenido.referencias.append(str_elem)
            fase = "referencias"
            continue

        if "como_citar" in clases or "iijunam" in clases_limpias or "apa" in clases_limpias or texto_lower == "cómo citar":
...
def generar_html_referencia(contenido: ContenidoArticulo, css_inline: str, nombre_revista: str) -> str:
    ...
    # SVG Configurado como inline-block y alineado perfectamente
    ORCID_SVG = """<a href="{url}" target="_blank" style="text-decoration: none; margin-left: 6px; display: inline-block; vertical-align: baseline; line-height: 1;"><span class="_idSVGInline" style="display: inline-block; width: 16px; height: 16px; vertical-align: text-bottom;"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="16" height="16" style="width: 16px !important; height: 16px !important; display: block;"><style type="text/css">.st0{fill:#A6CE39;}.st1{fill:#FFFFFF;}</style><circle class="st0" cx="128" cy="128" r="128"/><g><path class="st1" d="M86.3,186.2H70.9V79.1h15.4v48.4V186.2z"/><path class="st1" d="M108.9,79.1h41.6c39.6,0,57,28.3,57,53.6c0,27.5-21.5,53.6-56.8,53.6h-41.8V79.1z M124.3,172.4h24.5c34.9,0,42.9-26.5,42.9-39.7c0-21.5-13.7-39.7-43.7-39.7h-23.7V172.4z"/><path class="st1" d="M88.7,56.8c0,5.5-4.5,10.1-10.1,10.1c-5.6,0-10.1-4.6-10.1-10.1c0-5.6,4.5-10.1,10.1-10.1C84.2,46.7,88.7,51.3,88.7,56.8z"/></g></svg></span></a>"""
    
    autores_html_list = []
    for autor in contenido.autores_obj:
        orcid_str = ORCID_SVG.replace("{url}", autor.orcid_url) if autor.orcid_url else ""
        
        # Eliminación de saltos de línea basura al final del autor para forzar el ORCID a la misma línea
        nombre_limpio = re.sub(r'<br\s*/?>', '', autor.nombre_html).strip()
        autores_html_list.append(f'<p class="AUT-DOS-NOMBRES">{nombre_limpio}{orcid_str}</p>')
