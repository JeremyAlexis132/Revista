...
errores tipográficos en DOI, fechas fragmentadas, neutralizando saltos de línea 
forzados y corrigiendo el bug de referencias en negritas.
"""
...
def _limpiar_interior_autor(html_str: str) -> str:
    """Extrae únicamente el texto plano del autor eliminando cualquier etiqueta interna o estilos basura"""
    if not html_str:
        return ""
...
def extraer_contenido(html_path: str) -> ContenidoArticulo:
    ...
    # Remover atributos en línea que rompen el diseño
    for tag in soup.find_all(True):
        if tag.has_attr('style'):
            tag['style'] = re.sub(r'(font-family|font-size|text-align|font-weight)\s*:[^;]+;?', '', tag['style'], flags=re.IGNORECASE)
    ...
    for elem in elementos:
        ...
        if "aut-dos-nombres" in clases or "aut" in clases.split() or "autor" in clases_limpias:
            if autor_actual:
        ...
        if ("resumen" in clases and "resumen_ingles" not in clases) or texto_lower.startswith("resumen:"):
            contenido.resumen = str(elem)
            fase = "cuerpo"
            continue
            
        if "palabras-clave" in clases or texto_lower.startswith("palabras clave:"):
            contenido.palabras_clave = str(elem)
            continue

        if "resumen_ingles" in clases or texto_lower.startswith("abstract:"):
            contenido.abstract = str(elem)
            continue

        if "keywords" in clases or texto_lower.startswith("keywords:"):
            contenido.keywords = str(elem)
            continue

        if "referencias" in clases or "bib" in clases.split():
            # CORRECCIÓN DE REFERENCIAS EN NEGRITA
            # Si InDesign envolvió toda la referencia en <strong> o <b>, lo quitamos
            fuerte = elem.find(["strong", "b"])
            if fuerte and len(fuerte.get_text(strip=True)) > len(texto_limpio) - 5:
                fuerte.unwrap()
        ...
        # =======================================================
        # DETECCIÓN DE SECCIONES (Para evitar texto pequeño/centrado)
        # =======================================================
        if texto_limpio or elem.name in ["table", "img", "hr"]:
            if elem.name == "table":
                ...
                is_heading = False
                clases_str = " ".join(clases_limpias)
                
                if any(x in clases_str for x in ["romano", "arabigo", "seccion", "vv", "ia"]):
                    is_heading = True
                        
                if len(texto_limpio) < 150 and elem.name == "p":
                ...
                if is_heading:
                    if re.match(r'^(I|V|X)', texto_limpio) or "romano" in clases_str or "vv" in clases_str:
                        elem.name = "h3"
                        elem['class'] = ['romanos']
                    else:
                        elem.name = "h4"
...
def generar_html_referencia(contenido: ContenidoArticulo, css_inline: str, nombre_revista: str) -> str:
    ...
    # SVG Adaptado al estilo RMDE
    ORCID_SVG = """<a href="{url}" target="_blank" style="text-decoration: none; margin-left: 6px; display: inline-block; vertical-align: middle; line-height: 1;"><span class="_idSVGInline" style="display: inline-block; width: 16px; height: 16px; vertical-align: middle;"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="16" height="16" style="width: 16px !important; height: 16px !important; display: inline-block;"><style type="text/css">.st0{fill:#A6CE39;}.st1{fill:#FFFFFF;}</style><circle class="st0" cx="128" cy="128" r="128"/><g><path class="st1" d="M86.3,186.2H70.9V79.1h15.4v48.4V186.2z"/><path class="st1" d="M108.9,79.1h41.6c39.6,0,57,28.3,57,53.6c0,27.5-21.5,53.6-56.8,53.6h-41.8V79.1z M124.3,172.4h24.5c34.9,0,42.9-26.5,42.9-39.7c0-21.5-13.7-39.7-43.7-39.7h-23.7V172.4z"/><path class="st1" d="M88.7,56.8c0,5.5-4.5,10.1-10.1,10.1c-5.6,0-10.1-4.6-10.1-10.1c0-5.6,4.5-10.1,10.1-10.1C84.2,46.7,88.7,51.3,88.7,56.8z"/></g></svg></span></a>"""
    
    autores_html_list = []
    for autor in contenido.autores_obj:
        orcid_str = ORCID_SVG.replace("{url}", autor.orcid_url) if autor.orcid_url else ""
        
        # Limpieza de atributos para el nombre (Estilo RMDE)
        nombre_limpio = _limpiar_interior_autor(autor.nombre_html)
        autores_html_list.append(f'<p class="autor_final_2apellidos ORCID2">{nombre_limpio}{orcid_str}</p>')
        
        for adscripcion in autor.adscripciones_html:
            adscripcion_limpia = _limpiar_interior_autor(adscripcion)
            if "@" in adscripcion_limpia.lower() or "correo" in adscripcion_limpia.lower():
                autores_html_list.append(f'<p class="correo">{adscripcion_limpia}</p>')
            else:
                autores_html_list.append(f'<p class="adscripcion">{adscripcion_limpia}</p>')
